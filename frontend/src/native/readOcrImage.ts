...
export function flattenOcrText(result: {
  text?: string;
  blocks?: {
    text?: string;
...
export type ReadOcrImageFailure =
  | 'cancelled'
  | 'app_error'
  | 'unreadable';

export type ReadOcrImageResult =
  | { ok: true; text: string }
  | { ok: false; reason: ReadOcrImageFailure };

async function recognizeTextFromPath(path: string): Promise<ReadOcrImageResult> {
  ...
  const text = flattenOcrText(result);
  if (!text.trim()) {
...
export async function readOcrFromGallery(): Promise<ReadOcrImageResult> {
  ...
  try {
    ...
    return await recognizeTextFromPath(path);
  } catch (err) {
...
export async function readOcrFromCamera(): Promise<ReadOcrImageResult> {
  if (!Capacitor.isNativePlatform()) {
    return { ok: false, reason: 'app_error' };
  }
